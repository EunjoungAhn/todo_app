...
class DoneAllHistory extends StatefulWidget {
  const DoneAllHistory({Key key}) : super(key: key);

  @override
  State<DoneAllHistory> createState() => _DoneAllHistoryState();
}

class _DoneAllHistoryState extends State<DoneAllHistory> {
  int selectIndex = 1; // 네비게이션 변경되는 인덱스 넘버

  StreamController<_DoneAllHistoryState> streamController = StreamController<_DoneAllHistoryState>();
  ...
  // 모든 완료 메모들을 가져와라
  void getDoneAllTodo() async {
    allTodo = await dbHelper.getDoneAllTodo();
    setState(() { });
  }
  ...
  Widget _buildBody(BuildContext context){
    return FutureBuilder(
      future: DatabaseHelper.instance.getDoneAllTodo(),
